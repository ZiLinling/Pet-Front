	...
	<view class="favor">
		<view class="header">
			<uni-row class="category">
				<uni-col :span="8">
					<view class="pets" @tap="showPets()">宠物({{petsList.length}})</view>
				</uni-col>
				<uni-col :span="8">
					<view class="pets" @tap="showGoods()">周边({{goodsList.length}})</view>
				</uni-col>
				<uni-col :span="8">
					<view class="pets" @tap="showShops()">商店({{shopsList.length}})</view>
				</uni-col>
			</uni-row>
		</view>
			...
			<view class="product-list">
				...
				<view class="product" v-for="product in productList" :key="product.id" @tap="toGoods(product)">
					...
					<view class="name">
						...
						<view class="productName" v-if="favNow==1">{{product.breed_id}}</view>
						<view class="productName" v-if="favNow!=1">{{product.name}}</view>
					</view>
					<view class="fav">
					...
					<view class="info">
						...
						<view class="storeName" v-if="favNow==2">{{product.store_id}}</view>
						<view class="price" v-if="favNow!=3">${{ product.price }}</view>
						<!-- 占位符 -->
	...
	export default {
		...
		methods: {
			...
			showPets() {
				// 设置当前收藏的种类
				this.favNow = 1
				this.productList = this.petsList
			},
			showGoods() {
				// 设置当前收藏的种类
				this.favNow = 2
				this.productList = this.goodsList
			},
			showShops() {
				// 设置当前收藏的种类
				this.favNow = 3
				this.productList = this.shopsList
			},
